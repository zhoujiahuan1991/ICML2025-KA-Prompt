...
class ImageNet_C(Dataset):
    ...
    def __init__(self, domain_id=0, mode='train', transform=None,
        ...
        self.image_list_root = "datasets/image_list/imagenet-c_list"
        self.task_name = ['defocus_blur', 'glass_blur', 'motion_blur', 'zoom_blur',
...
class SequentialImageNet_C(ContinualDataset):
    ...
    def get_finetune_all_dataloader(self, mode="train"):
        dataset = ImageNet_C_ALL(mode=mode)
        if mode == "train":
            shu = True
        else:
            shu = False
    ...
    def get_dataset(self, task_id, mode='train'):
        return ImageNet_C(domain_id=task_id, mode=mode, 
                                    transform=SequentialImageNet_C.TRANSFORM_DOMAIN_NET)
                                    # transform=SequentialImageNet_C.TRANSFORM_DOMAIN_NET)

    def get_test_dataloaders(self, task_id):
        test_loader_all = []
        for i in range(task_id + 1):
            test_dataset = ImageNet_C(domain_id=i, mode='test', 
                                        transform=SequentialImageNet_C.get_trans_test(self.args))
                                        # transform=SequentialImageNet_C.TRANSFORM_DOMAIN_NET)
    ...
    def get_data_loaders(self, task_id,eval=False):
        if eval:
            train_dataset = ImageNet_C(domain_id=task_id, mode='train', 
                                        transform=SequentialImageNet_C.get_trans_test(self.args))
            ...
            train_dataset = ImageNet_C(domain_id=task_id, mode='train', 
                                        transform=SequentialImageNet_C.get_trans_train(self.args))
                                        # transform=SequentialImageNet_C.TRANSFORM_DOMAIN_NET)
            train_loader = DataLoader(train_dataset,
    ...
    def get_transform():
        transform = transforms.Compose(
            [transforms.ToPILImage(), SequentialImageNet_C.TRANSFORM])
        return transform
    ...
    def get_backbone():
        ...
        model.head = torch.nn.Linear(768,SequentialImageNet_C.N_CLASSES)
        return model
